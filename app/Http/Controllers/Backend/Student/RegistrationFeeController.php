   ...
   public function RegFeeClassData(Request $request){
    	...
    	//  $html['thsource'] .= '<th>Student Name</th>';
    	 ...
    	 foreach ($allStudent as $key => $v) {
    	 	...
    	 	$html[$key]['tdsource'] .= '<td>'.$v['id'].'</td>';
    	 	// $html[$key]['tdsource'] .= '<td>'.$v['name'].'</td>';
    	 	$html[$key]['tdsource'] .= '<td>'.$v['student_id'].'</td>';
    ...
    public function RegFeePayslip(Request $request){
    	...
    	$allStudent['details'] = AssignStudent::with(['discount'])->where('student_id',$student_id)->where('class_id',$class_id)->first();


	$pdf = new Dompdf();
		$pdf->loadHtml(view('backend.student.registration_fee.registration_fee_pdf', $allStudent));
