...
class StudentRollController extends Controller
{
    ...
    public function GetStudents(Request $request){
    	//dd('ok done');
    	$allData = students::where('year_id',$request->year_id)->where('class_id',$request->class_id)->get();
    	// dd($allData->toArray());
    ...
    public function StudentRollStore(Request $request){
    	...
    	if ($request->student_id !=null) {
    		for ($i=0; $i < count($request->student_id); $i++) { 
    			students::where('year_id',$year_id)->where('class_id',$class_id)->where('student_id',$request->student_id[$i])->update(['roll' => $request->roll[$i]]);
    		} // end for loop
    	}else{
    		$notification = array(
