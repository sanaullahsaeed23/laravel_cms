...
class StudentFeeController extends Controller
{
  ...
  public function StudentFeeGetStudent(Request $request){
	...
	// $data =students::where('rollNumber',$rollNumber)->where('class_id',$class_id)->get();
	// <input type="text" name="rollNumber" required class="form-control" placeholder="Enter Roll Number">
	// $year_id = $request->year_id;
   	$class_id = $request->class_id;
   	$fee_category_id = $request->fee_category_id;
   	$date = date('Y-m',strtotime($request->date));    	   
    	 
  $data = AssignStudent::with(['student','discount'])->where('student_id',$rollNumber)->where('class_id',$class_id)->get();
